	...
	private boolean handleUpdateEntry(String platform, String key, Map.Entry<String, String> entry) throws IOException {
		...
		try {
			...
			boolean hasAnyValidPhrase =
				hasSpigotPhrase || hasGithubPhrase || hasJenkinsPhrase || hasBukkitPhrase || hasModrinthPhrase || hasHangarPhrase || blobBuildPhrase ||
					busyBiscuitPhrase;
		...
		try {
			...
			if (value.endsWith("/dev/")) {
				...
				if (value.contains(multiIdentifier)) {
					...
					int endIndex = value.indexOf("]", startIndex);
					artifactNum = Integer.parseInt(value.substring(startIndex + 1, endIndex));
					repoPath = getGitHubRepoLocation(value.substring(0, value.indexOf(multiIdentifier)));
				} else {
					repoPath = getGitHubRepoLocation(value.substring(0, value.length() - 4));
				}
			...
			if (value.contains(multiIdentifier)) {
				...
				repoPath = getGitHubRepoLocation(value);
			}
